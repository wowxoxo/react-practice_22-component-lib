...
// Casts
const firstSquare1 = shapes.find(shape => shape.type === "SQUARE") as Square | undefined
...
const isRectangle1 = (shape: Shape): shape is Rectangle => shape.type === "RECTANGLE"
...
const sizeOneSquare = shapes.find(shape => isSquare1(shape) && shape.size === 1)
console.log(sizeOneSquare?.size)

const sizeOneSquare1 = shapes.filter(isSquare1).find(shape => shape.size === 1)
...
const personWithAddress = (p: Person): p is PersonWithAddress => p.address !== undefined
